<div class="item popular-item">
	<div class="thumbnail">
		<a href="{{route('services_details',[$sponser->service->user->username,$sponser->service->seo_url])}}">
			@php 
			$image_url = url('public/frontend/assets/img/No-image-found.jpg');
			@endphp
			@if(isset($sponser->service->images[0]))
			@if($sponser->service->images[0]->photo_s3_key != '')
			@php 
	...
	<div class="product-info">
		<a href="{{route('services_details',[$sponser->service->user->username,$sponser->service->seo_url])}}">
			<h4 class="text-header min-title-height text-capitalize">
				{{display_title($sponser->service->title, 35)}}            
			</h4>
		</a>

		<p class="product-description min-description-height">
			{{display_subtitle($sponser->service->subtitle, $sponser->service->descriptions, 50)}}
		</p>
