<form id="portfolio_form" name="portfolio_form" action="{{ route('portfolio.update',$portfolio->secret) }}" method="post">
	@csrf
	...
	<div class="modal-body pt-0 border-0 px-3 px-md-5">
		<h3 class="font-weight-bold font-20 text-color-6 text-center">Edit Project</h3>
		<div class="form-group">
			<p class="mb-1 font-16 text-color-6 mt-5 font-weight-bold">Project Name</p>
			<input name="title" id="title" value="{{$portfolio->title}}" type="text" class="font-14 form-control summary" placeholder="Enter your project name">
		</div>
		<div class="form-group description">
			<p class="mb-1 font-16 text-color-6 mt-3 font-weight-bold">Description</p>
			<textarea name="description" class="summary font-14 w-100 p-2 bg-transparent" cols="30" rows="4" placeholder="Tell us more about the project">{{$portfolio->description}}</textarea>
		</div>
			...
			<div class="media-upload-form" style="display:none;"> 
				<div class="dropzone fileinput-button dz-clickable custome-dropzone-media-upload">
					<span class="dz-default dz-message text">
						...
						<div class="form-group">
							<input type="hidden" name="upload_media" id="upload_media" value="{{$portfolio->media_link}}">
				...
				<div class="d-flex justify-content-between align-items-center">
					<div class="d-flex align-items-center">
						@if($portfolio->media_type == 'image')
							<img class="img-fluid cart-logo media_show" src="{{$portfolio->thumbnail_url}}" />
						@else
						<img src="{{($portfolio->thumbnail_url)? $portfolio->thumbnail_url : url('public/frontend/images/video_players.png')}}" class="img-fluid cust-portfolio-video-thumb img-fluid cart-logo media_show">
						@endif
						<div class="mx-3">
							<p class="mb-0 font-16 text-color-2 font-weight-bold media_name">{{$portfolio->original_name}}</p>
							<p class="mb-0 font-14 text-color-4 media_size">{{$portfolio->media_size}}</p>
						</div>  
					</div>
