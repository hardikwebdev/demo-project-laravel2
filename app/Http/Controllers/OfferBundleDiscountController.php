...
class OfferBundleDiscountController extends Controller
{
    private $uid;
    ...
    {
     	$model = BundleDiscount::where(['user_id' => $this->uid])
        ->where('type',0)
    ...
    public function create(){
    	$serviceList = Service::where('uid',$this->uid)
    ...
    public function store(Request $request){
        ...
        for($i=0;$i<count($request->service_ids);$i++)
        {
            ...
            if($checkServiceId)
            {
                ...
                return Redirect::back()->withErrors(['Service is already added in other combo']);
            }
        }
        ...
        if($jsonResponse['valid']){
            ...
            return redirect()->route('offer_bundle_discount')->with('errorSuccess',$message);
        }else{
    ...
    public function edit(Request $request,$id){
        $model = BundleDiscount::where('user_id',$this->uid)->where('type',0)->find($id);
        if(count($model) > 0){
            $selectedServiceIds = [];
    ...
    public function update(Request $request){
        ...
        if($jsonResponse['valid']){
            $model = BundleDiscount::where('user_id',$this->uid)->where('type',0)->find($request->id);
            if(count($model) > 0){
                $model->discount = $request->discount;
                ...
                BundleService::where('bundle_id',$model->id)->where('type',0)->delete();
                if($request->filled('service_ids') && count($request->service_ids) > 0){
                    foreach ($request->service_ids as $key => $value) {
                        $bundleService = new BundleService;
                        $bundleService->bundle_id = $model->id;
    ...
    public function checkDiscount(Request $request){
        ...
        if($request->filled('service_ids') && count($request->service_ids) > 0 && $request->filled('discount') && $request->discount !=''){
            ...
            $service = Service::where('uid',$this->uid)
            ->where('is_custom_order', 0)
            ->where('is_recurring', 0)
            ->where('is_job',0)
            ->where('is_approved', 1)
    ...
    public function delete(Request $request,$id){
        $model = BundleDiscount::where('type',0)->where('user_id',$this->uid)->find($id);
        if($model){
